...
class BTLEBusyError(BTLEException):
    def __init__(self, message, rsp=None):
        BTLEException.__init__(self, message, rsp)
...
class UUID:
    def __init__(self, val, commonName=None):
        ...
        if len(self.binVal) != 16:
            raise ValueError(
                "UUID must be 16 bytes, got '%s' (len=%d)" % (val, len(self.binVal))
            )
...
class Peripheral(BluepyHelper):
    ...
    def _connect(self, addr, addrType=ADDR_TYPE_PUBLIC, iface=None, timeout=None):
        ...
        while rsp and rsp["state"][0] in ["tryconn", "scan"]:
            rsp = self._getResp("stat", timeout)
        if rsp is None:
            self._stopHelper()
        ...
        if rsp["state"][0] == "disc":
            self._stopHelper()
            raise BTLEBusyError(
                "Bluepyhelper is busy. Failed to connect to peripheral %s, addr type: %s"
                % (addr, addrType),
                rsp,
            )
        if rsp["state"][0] != "conn":
    ...
    def discoverServices(self):
        ...
        for i in range(nSvcs):
            self._serviceMap[UUID(uuids[i])] = Service(
                self, uuids[i], starts[i], ends[i]
            )
    ...
    def getCharacteristics(self, startHnd=1, endHnd=0xFFFF, uuid=None):
        ...
        return [
            Characteristic(
                self, rsp["uuid"][i], rsp["hnd"][i], rsp["props"][i], rsp["vhnd"][i]
            )
    ...
    def setRemoteOOB(self, address, address_type, oob_data, iface=None):
        ...
        if address_type not in (ADDR_TYPE_PUBLIC, ADDR_TYPE_RANDOM):
            raise ValueError(
                "Expected address type public or random, got {}".format(address_type)
            )
    ...
    def getLocalOOB(self, iface=None):
        ...
        if resp is not None:
            ...
            return {
                ...
                "Type": "".join(
                    ["%02X" % struct.unpack("<B", c)[0] for c in address_type]
                ),
    ...
    def getValue(self, sdid):
        ...
        if sdid in [ScanEntry.SHORT_LOCAL_NAME, ScanEntry.COMPLETE_LOCAL_NAME]:
            try:
                ...
                bbval = bytearray(val)
                return "".join(
                    [(chr(x) if (x >= 32 and x <= 127) else "?") for x in bbval]
                )
        elif sdid in [
    ...
    try:
        for svc in conn.services:
            ...
            for ch in svc.getCharacteristics():
                print(
                    "    {}, hnd={}, supports {}".format(
                        ch, hex(ch.handle), ch.propertiesToString()
                    )
                )
